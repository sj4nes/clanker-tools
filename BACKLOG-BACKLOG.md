...
| `test-writing` | **promoted to near-term — see [`BACKLOG.md`](BACKLOG.md).** what to test and why — property vs example, the test pyramid, test smells, coverage ≠ confidence, mutation testing | a fixture with planted bugs; the prescribed tests must catch them, mutation score checked | promoted |
| `test-oracle-design` | **retired 2026-09-13 — folded into `test-writing`, not split out.** The deciding evidence was this row's own *Verify* column: the fixture it proposed is the one `skills/test-writing/verification/` already ships, subject for subject, and two skills cannot share one verification fixture and stay independently falsifiable. Its content lives in `test-writing` behaviour 3, which now names the four oracle routes (spec re-derivation, model-based reference oracle, metamorphic relation, differential testing against a genuinely independent implementation) and ends on the independence test — *does my oracle share code with the thing it is judging?* New fixture subject F plants the trap that makes that falsifiable: two renderers with different algorithms sharing one defective helper, wrong identically, differential test green. | — | retired |
...
| Candidate | One line | Verify | Confidence |
|---|---|---|---|
| `methodology-skill-builder` | the meta-skill for building an analysis-methodology skill — the shape `simulation` / `DOE` / `control-systems` / `unknown-discovery` all share, extracted. **Must emit the displacement table** of [`docs/verifying-skills.md`](docs/verifying-skills.md) §7 as a required artifact (section → default behaviour displaced → where that default visibly fails) and refuse a section that neither displaces a nameable default nor declares itself `judgement` — the operational form of the eval meta-finding that tutorial-style skills underperform no skill at all. | build a small new skill with it; it must pass the same checks the hand-built ones do, **and** its displacement table must have no unmarked rows | plausible |
