...
| Candidate | One line | Verify | Confidence |
|---|---|---|---|
| `chemistry-reaction-thermo-kinetics` | thermodynamics of reactions → equilibrium → rate laws → Arrhenius → catalysis | `bc` equilibrium-constant and rate-integration checks | speculative |
...
| `theorem-tree-tutorial` | the math analogue of `formula-tree-tutorial` — turn a `math-theorem-tree` capsule into an `upmd`-runnable lesson (the capsules already ship tutorials; nothing *builds* them) | `upmd --ci --all` green on a generated tutorial | plausible |
