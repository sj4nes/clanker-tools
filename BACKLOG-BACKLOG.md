...
| `test-oracle-design` | narrower, more-checkable slice of `test-writing`: how you decide an output is *correct* — independent re-derivation in a fresh context (no helper reuse), a model-based reference oracle, metamorphic relations (related inputs → predictable output relationship), and *true* differential testing (two independently built implementations, never the same code twice). Motivated by danluu's "how well do agents use test techniques" eval (2026-09): agents overwhelmingly encode the code's own output as the expected value and never build a separate oracle. | fixture with planted bugs of the masking shapes — palindromic/symmetric input data hiding reversal & transposition bugs, output-encoded expectations, naive randomization that only hits rejection paths; the prescribed oracle must catch each and a negative-contrast naive test must miss it | plausible |
