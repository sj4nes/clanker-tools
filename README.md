...
| `physics-formula-tree` | verified via `physics-newtonian` | Method exercised end-to-end building the 58-node Newtonian capsule. Fixes folded back: `bc` rejects uppercase names (use lowercase `[M L T]`-exponent triples); Lean without Mathlib has no `ring`, so fall back to kernel-`decide`d `Int` instance checks and record what stays unproven. BSD `tsort` cycle check (from the `tsort` skill) confirmed necessary. |
