...
regset = set(reg)
reg_type = {l.split(chr(9))[0]: l.split(chr(9))[1] for l in open("nodes/nodes.tsv").read().splitlines()[1:] if l.strip()}
...
for nid in reg:
    ...
    if not (d.get("specialization_cases")) and d.get("status_label") != "proved_theorem":
        # cited bridge roots (status proved_theorem, lean_status cited) carry their
        # specialization cases in the source capsule
        if (reg_type.get(nid) != "bridge"):
