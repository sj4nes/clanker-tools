...
history rather than every change. The full record is
    ...
    git log -- skills/math-real-analysis/

## 2.1.0 — 2026-09-14

**MINOR.** Four nodes carried `lean_status: core-arith`, a label outside the
documented vocabulary, now `partial`. The refs pointed at real, existing
sections, so the claim was true — it just could not be checked. (`abb9d0f`)
...
use. Separately, **4 `results/*.yaml` did not parse at all** (unquoted `[a,b]`
in a flow mapping, a `:` inside a plain scalar); this capsule had no
