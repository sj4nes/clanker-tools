# Changelog — physics-thermoacoustics
...
history rather than every change. The full record is
    ...
    git log -- skills/physics-thermoacoustics/
...
use. Five formula entries also disagreed with the graph, one holding prose
where the `Prereqs:` list belongs. Also new: `validation/mutation-check.sh`,
which plants five graph defects per build and asserts each is caught — hygiene
checks pass a deleted *or* an invented edge. (`904ea6a`)
...
First release, verified at release. (`eb67ed8`)
