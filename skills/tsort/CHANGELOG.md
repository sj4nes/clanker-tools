...
history rather than every change. The full record is
    ...
    git log -- skills/tsort/

## 2.0.0 — 2026-09-13

**MAJOR.** The skill said to detect a cycle by checking `tsort`'s exit status.
BSD `tsort` **exits 0** on a cycle and writes to stderr, so any pipeline built
on that guidance had no cycle detection at all. Recorded in the version
re-base (`0fbb03c`, adjudicated `f921886`); see [`docs/skill-versioning.md`
