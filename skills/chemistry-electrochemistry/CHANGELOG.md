# Changelog — chemistry-electrochemistry
...
history rather than every change. The full record is
    ...
    git log -- skills/chemistry-electrochemistry/
...
use. The `nernst_equation` entry also omitted `dilute_ideal_solution` from its
`Prereqs:` line. Also new: `validation/mutation-check.sh`, which plants five
graph defects per build and asserts each is caught — hygiene checks pass a
deleted *or* an invented edge. (`904ea6a`)
...
First release, verified at release. (`a86fecf`)
