...
N("continuity_of_probability", "proposition", "probability_space",
  ...
  proof=("measure_continuity with mu = P; from above via complements", "measure_continuity", 'cited', None),
...
N("expectation_monotonicity", "proposition", "expectation",
  ...
  proof=("integral of a nonnegative function is >= 0; apply to Y - X; the triangle inequality |E X| <= E|X| from -|X| <= X <= |X|", "abstract_integral", 'cited', None),
...
N("covariance_bilinear", "proposition", "moments",
  ...
  proof=("expand the centered products and use expectation_linearity in each slot; symmetry is commutativity of the product; Cov(X,X) = Var(X) >= 0", "expectation_linearity", 'core', 'validation/proof-checks.lean Prob.cov_bilinear_raw (GENUINE raw-moment bilinearity)'),
...
N("multiplication_rule", "identity", "independence",
  ...
  proof=("P(A|B) P(B) = [P(A cap B)/P(B)] P(B) = P(A cap B); induct for the n-fold version", "conditional_probability", 'cited', None),
...
N("mgf_sum_independent", "identity", "independence",
  ...
  proof=("E[e^{t(X+Y)}] = E[e^{tX} e^{tY}] = E[e^{tX}] E[e^{tY}] by independence_expectation applied to g(X) = e^{tX}, h(Y) = e^{tY}", "independence_expectation", 'cited', None),
...
N("holder_inequality", "theorem", "inequalities",
  ...
  proof=("normalize, apply Young's inequality (from convexity of exp / concavity of log) pointwise, integrate", "jensen_inequality", 'cited', None),
...
N("poisson_distribution", "definition", "distributions",
  ...
  proof=("mean: sum k e^{-lambda} lambda^k/k! = lambda sum e^{-lambda} lambda^{k-1}/(k-1)! = lambda. Variance via E[X(X-1)] = lambda^2 then Var = lambda^2 + lambda - lambda^2. MGF: sum e^{tk} e^{-lambda} lambda^k/k! = e^{-lambda} e^{lambda e^t}", "series_convergence", 'none', None),
...
N("exponential_distribution", "definition", "distributions",
  ...
  proof=("normalization integral_0^inf lambda e^{-lambda x} dx = 1; E[X] = integral x lambda e^{-lambda x} dx = 1/lambda by parts; memorylessness from P(X > t) = e^{-lambda t}", "pdf", 'none', None),
...
N("normal_distribution", "definition", "distributions",
  ...
  proof=("normalization by the Gaussian integral integral e^{-x^2/2} dx = sqrt(2 pi); MGF by completing the square; E = mu, Var = sigma^2 from M''(0)", "abstract_integral", 'none', None),
...
N("standard_normal", "definition", "distributions",
  ...
  proof=("Z = (X - mu)/sigma for X ~ N(mu, sigma^2) via transformation_univariate; phi_Z(t) = e^{-t^2/2} by completing the square in E[e^{itZ}]", "transformation_univariate", 'none', None),
...
N("normal_affine_closure", "proposition", "distributions",
  ...
  proof=("affine: change of variables on the density. Sum: mgf_sum_independent then mgf_uniqueness -- the exponents add", "mgf_sum_independent", 'cited', None),
...
N("tower_property", "theorem", "conditional_expectation",
  ...
  proof=("for A in H subset G: integral_A E[E[X|G]|H] = integral_A E[X|G] = integral_A X = integral_A E[X|H]; both outer terms are H-measurable with equal integrals on all H-sets, so equal a.s.", "conditional_expectation_existence", 'cited', None),
