...
full N(...) spec.  This capsule carries TWO per-result tags (see scope.md):
  field_scope  -- any_field | char_not_2 | ordered_field | real_or_complex |
                  algebraically_closed   (the analogue of math-statistics'
                  `regime`), and
  choice_grade -- choice_free | needs_countable_choice | needs_full_AC
                  (inherited from math-sets-functions-cardinality).
"""
import os, sys
...
DEPS = load_deps()
try:
    from leanmap import LEAN_MAP
except ImportError:
    LEAN_MAP = {}

DEFAULT_SOURCES = ["axler_lada_4e", "hoffman_kunze_2e"]
...
class N:
    ...
    def __init__(self, nid, ntype, area, display, parseable="", symbols=None,
                 ...
                 field_scope=None, choice_grade='choice_free', solving_for=None, spec=None, cxd=None, misuse=None,
        ...
        self.field_scope = field_scope; self.choice_grade = choice_grade
        self.solving_for = solving_for
        ...
        # AUTHORITATIVE lean reconciliation: build/leanmap.py overrides every
        # spec claim.  Nodes absent from the map are forced to `cited` -- a spec
        # cannot overclaim a Lean status it has not earned.
        if self.proof is not None:
            tech, derives, _lstat, _lref = self.proof
            if nid in LEAN_MAP:
                _lstat, _lref = LEAN_MAP[nid]
            elif _lstat in ("core", "dim_core", "instance", "partial"):
                _lstat, _lref = "cited", None
            self.proof = (tech, derives, _lstat, _lref)
    ...
    def yaml(self):
        ...
        if self.field_scope:
            o += f"field_scope: {self.field_scope}\n"
        o += f"choice_grade: {self.choice_grade}\n"
    ...
    def md(self):
        ...
        if self.field_scope:
            o += f"  ·  field_scope: {self.field_scope}"
        if self.choice_grade != "choice_free":
            o += f"  ·  choice_grade: {self.choice_grade}"
...
def ROOT(nid, cite, display):
    ...
    N(nid, "bridge", "foundations", display,
      ...
      sources=[cite.split(":")[0]], status="draft", field_scope=None,
