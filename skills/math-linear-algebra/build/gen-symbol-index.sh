...
"$PTX" -A -W '[A-Za-z_][A-Za-z0-9_]*' build/_corpus.txt > build/_ptx.raw 2>/dev/null || true
{
  ...
  echo
  for kw in basis dimension span independent rank nullity kernel image \
            determinant minor cofactor adjugate permutation eigenvalue eigenvector \
            characteristic minimal diagonalisable nilpotent Jordan Schur \
            orthogonal orthonormal projection adjoint unitary Hermitian symmetric \
            definite semidefinite Cholesky Gram singular pseudoinverse condition \
            trace transpose invertible echelon pivot congruence inertia norm; do
    printf '## %s\n' "$kw"
