...
| 7. Dimensional checks + limiting cases | **bc** | dimension exponents of LHS/RHS; limiting-case expressions (`v=0`, `v/c→0`, leading corrections) | exact decimal arithmetic, explicit precision policy, explicit rounding (bc truncates). Record status in the formula YAML `dimension_check_status` and `validation/dimensional-checks.md`. **bc has no vectors and rejects uppercase names** — represent each quantity as three lowercase scalars (`<q>m <q>l <q>t` for the `[M L T]` exponents) and let each check print `0 0 0` for LHS−RHS. A single `!= 0` component is a real inconsistency (or a typo in the check — the zero-gate catches both). |
