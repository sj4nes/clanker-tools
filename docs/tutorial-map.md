...
2. ~~**√2 is irrational — and that's a crisis**~~ — ✅ **shipped 2026-09-07** as [`building-the-number.md`](../skills/math-number-systems/tutorial/building-the-number.md). 24 blocks; exercised the skill on a *constructions* capsule — the well-definedness beat (`cx_` = a broken operation on classes) is the new pattern. Fixed a pre-existing parse bug in `cantor_diagonal_argument.yaml`. (2026-09-08: added `app_sqrt2_irrational` — the p-adic valuation criterion `√n ∈ ℚ ⟺ every vₚ(n) even`, checked against brute square-testing.)
