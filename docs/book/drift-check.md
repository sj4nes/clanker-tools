...
**Fix:** now cheap. `tools/corpus.py` already reads the corpus, and the Part IV/V
generator and freshness gate already exist; the counts in Part I can be
generated into the prose the same way, which retires the row's `draft-anyway`
status instead of re-checking it by hand.
