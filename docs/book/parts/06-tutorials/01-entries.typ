...
#tutorialentry([The Voltage Isn't Fixed], [#raw("chemistry-electrochemistry") #sym.dot.c #raw("formula-tree-tutorial")],
  (
    ([teaches], [Install upmd, then from the repository root run #raw("upmd skills/chemistry-electrochemistry/tutorial/the-voltage-isnt-fixed.md") for the interactive walk, or #raw("upmd --ci --all skills/chemistry-electrochemistry/tutorial/the-voltage-isnt-fixed.md") to run every calculation top to bottom.]),
    ([runs], [9 runnable blocks #sym.dot.c 2 of them a Lean core #sym.dot.c calls #raw("bc"), #raw("lean"), #raw("awk")]),
...
#tutorialentry([Predicting the pH of an Acid — and Holding It with a Buffer], [#raw("chemistry-foundations") #sym.dot.c #raw("formula-tree-tutorial")],
  (
    ([teaches], [Install upmd, then from the repository root run #raw("upmd skills/chemistry-foundations/tutorial/predicting-ph.md") for the interactive walk, or #raw("upmd --ci --all skills/chemistry-foundations/tutorial/predicting-ph.md") to run every calculation top to bottom.]),
...
#tutorialentry([Building the number that isn't there], [#raw("math-number-systems") #sym.dot.c #raw("theorem-tree-tutorial")],
  (
    ([teaches], [Install #raw("upmd"), then from the repository root:]),
...
#tutorialentry([Designing an Organ Pipe], [#raw("physics-acoustics") #sym.dot.c #raw("formula-tree-tutorial")],
  (
    ([teaches], [Install #raw("upmd") (https://upmd.dev), then run #raw("upmd designing-an-organ-pipe.md") for the interactive walk, or #raw("upmd --ci --all designing-an-organ-pipe.md") to run every calculation top to bottom. Each section ends with a code block you run yourself; blocks depend on earlier ones, so #raw("upmd --ci -b capstone designing-an-organ-pipe.md") runs the full chain.]),
...
#tutorialentry([Are a Megaphone and a Long-Range Microphone the Same Thing?], [#raw("physics-acoustics") #sym.dot.c #raw("formula-tree-tutorial")],
  (
    ([teaches], [Install #raw("upmd") (https://upmd.dev), then run #raw("upmd horns-and-reciprocity.md") for the interactive walk, or #raw("upmd --ci --all horns-and-reciprocity.md") to run every calculation top to bottom. Each section ends with a code block you run yourself; #raw("upmd --ci -b capstone horns-and-reciprocity.md") runs the full chain.]),
    ([runs], [4 runnable blocks #sym.dot.c calls #raw("bc")]),
...
#tutorialentry([How Fast Does Sound Travel?], [#raw("physics-acoustics") #sym.dot.c #raw("formula-tree-tutorial")],
  (
    ([teaches], [Install #raw("upmd") (https://upmd.dev), then run #raw("upmd how-fast-does-sound-travel.md") for the interactive walk, or #raw("upmd --ci --all how-fast-does-sound-travel.md") to run every calculation top to bottom. Each section ends with a code block you run yourself; blocks depend on earlier ones, so #raw("upmd --ci -b chk_speed_of_sound_ideal_gas how-fast-does-sound-travel.md") runs a single check with its whole dependency chain.]),
...
#tutorialentry([Why a Pendulum Keeps Time], [#raw("physics-newtonian") #sym.dot.c #raw("formula-tree-tutorial")],
  (
    ([teaches], [Install upmd, then from the repository root run #raw("upmd skills/physics-newtonian/tutorial/pendulum.md") for the interactive walk, or #raw("upmd --ci --all skills/physics-newtonian/tutorial/pendulum.md") to run every calculation top to bottom.]),
...
#tutorialentry([Why Heat Engines Have a Ceiling], [#raw("physics-thermodynamics") #sym.dot.c #raw("formula-tree-tutorial")],
  (
    ([teaches], [Install #raw("upmd") (https://upmd.dev), then run #raw("upmd why-heat-engines-have-a-ceiling.md") for the interactive walk, or #raw("upmd --ci --all why-heat-engines-have-a-ceiling.md") to run every calculation top to bottom. Each section ends with a code block you run yourself; blocks depend on earlier ones, so #raw("upmd --ci -b capstone why-heat-engines-have-a-ceiling.md") runs the full chain.]),
    ([runs], [3 runnable blocks #sym.dot.c calls #raw("bc")]),
